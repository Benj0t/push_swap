...
static int		my_free(char **line, t_struct *v, int fd)
{
	if (v->tmp)
		free(v->tmp);
	if (v->s[fd])
...
static int		my_charset(char *str)
{
	int i;
...
static int		my_ret(t_struct *v, char **line, int fd, int bool)
{
	...
	{
		v->len = v->len - v->pos;
		if (!(*line = my_substr(v->s[fd], 0, v->pos, 0)))
			return (my_free(line, v, fd));
		if (!(v->tmp = my_substr(v->s[fd], v->pos + 1, v->len, 1)))
			return (my_free(line, v, fd));
		if (bool == 2 && !(v->s[fd] = my_strdup(v->tmp, v->len)))
			return (my_free(line, v, fd));
		if (bool == 1 && !(v->s[fd] = my_strdup(v->tmp, v->len)))
			return (my_free(line, v, fd));
		return (1);
	}
	return (-1);
...
static int		my_gnl(int fd, char **line, t_struct *v)
{
	while ((v->ret = read(fd, v->buffer, BUFFER_SIZE)) > 0 &&
			(v->pos = my_charset(v->buffer)) < 0)
	{
		v->buffer[v->ret] = '\0';
		v->len += v->ret;
		if (!(v->tmp = my_strjoin(v->s[fd], v->buffer, v->len)))
			return (my_free(line, v, fd));
		if (!(v->s[fd] = my_strdup(v->tmp, v->len)))
			return (my_free(line, v, fd));
	}
	v->len += v->ret;
	v->buffer[v->ret] = '\0';
...
int				get_next_line(int fd, char **line)
{
	...
	if (line == NULL || BUFFER_SIZE <= 0 || read(fd, NULL, 0) == -1)
		return (-1);
	if (!v.s[fd])
		if (!(v.s[fd] = my_calloc(0, 0)))
			return (my_free(line, &v, fd));
	if (!(v.buffer = my_calloc(1, BUFFER_SIZE + 1)))
		return (my_free(line, &v, fd));
	v.len = my_strlen(v.s[fd]);
	if ((v.pos = my_charset(v.s[fd])) >= 0)
	{
		free(v.buffer);
		v.buffer = NULL;
